...
function getShows() {
  ...
  // construct the table content into rowString variable
   ...
   for (let ndx=0; ndx < showList.length; ndx++) {
    if (deletedIndex.includes(ndx)) continue;

    if (cat == "All" || showList[ndx].category == cat) { // include only the selected shows based on category
        rowString += "<tr>";
        rowString += "<td>" + ndx + "</td>";
        rowString += "<td>" + showList[ndx].title + "</td>";
        rowString += "<td>" + showList[ndx].cast.join(", ") + "</td>";
        rowString += "<td>" + showList[ndx].year + "</td>";
        rowString += "<td>" + showList[ndx].ratingScore() + "</td>";
        rowString += "</tr>";
    }
   }
...
function deleteByCategory() {
  ...
  if (!confirm("Delete ALL shows under " + cat + "?")) return;

  for (let i = 0; i < getShowList().length; i++) {
    const showList = getShowList();
    if (showList[i].category == cat && !deletedIndex.includes(i)) {
      deletedIndex.push(i);
    }
  }
